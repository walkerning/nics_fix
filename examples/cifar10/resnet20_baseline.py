...
def ResNet20(x, num_classes, training, weight_decay):
    with nf.kwargs_scope_by_type(
        Conv2D={"padding": "same",
                "kernel_regularizer": tf.contrib.layers.l2_regularizer(scale=weight_decay),
                "kernel_initializer": tf.contrib.layers.variance_scaling_initializer()},
        MaxPool={"pool_size": (2, 2),
                 "strides": 2},
        BN={"is_training": training, "scale": True},
        Dense={"kernel_regularizer": tf.contrib.layers.l2_regularizer(scale=weight_decay),
               "kernel_initializer": tf.contrib.layers.variance_scaling_initializer()}):
...
def main(_):
    ...
    # Learning rate is multiplied by 0.1 after training for every 30 epochs
    ...
    if FLAGS.prune:
        train_step = nf.apply_gradients_prune(optimizer.apply_gradients, grads_and_vars, FLAGS.mask_file, global_step=global_step)
        log("INFO: pruning.")
    else:
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            for epoch in range(1, FLAGS.epochs+1):
                ...
                # Save Strategy and checkpoint
                if FLAGS.train_dir and epoch % 5 == 0:
                    ...
                    if FLAGS.save_strategy:
                        ...
                        with open(FLAGS.save_strategy + str(save_id), "w") as f:
                            yaml.dump(strategy_config_dct, f)
                        log("Dump hardware straetgy file to {}".format(FLAGS.save_strategy + str(save_id)))

                    if FLAGS.train_dir:
                        cpkt_path = FLAGS.train_dir + "/model" + str(save_id) + "/"
                        if not os.path.exists(cpkt_path):
                            subprocess.check_call("mkdir -p {}".format(cpkt_path),
                                                  shell=True)
                        log("Saved model to: ", saver.save(sess, os.path.join(cpkt_path, str(acc_1_test)), global_step=global_step))
                # End save strategy and checkpoint
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="ResNet20",
                        choices=["VGG11", "Conv4Dense2, ResNet20"], help="The network structure.")
