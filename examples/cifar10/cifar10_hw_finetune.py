...
def main(_):
    ...
    grads_and_vars = [[nf.quant._do_quantitize(grad, fixed_mapping[nf.DataTypes.WEIGHT][var]["q_data_scale"] - (tf.log(learning_rate) / np.log(2)), 24, "hi_"+var.op.name), var] \
    ...
    with tf.Session(config=config) as sess:
        ...
        if FLAGS.global_step >= 0:
            sess.run(tf.assign(global_step, FLAGS.global_step))

        log("Start training from step: {}...".format(sess.run(global_step)))
        log("Start training with learning rate: {:.5f}...".format(sess.run(learning_rate)))
        # Training
        ...
        try:
            for epoch in range(1, FLAGS.epochs+1):
                ...
                if FLAGS.train_dir and epoch == FLAGS.epochs:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global_step", type=int, default=-1,
                        help="The global step of the load checkpoint.")
